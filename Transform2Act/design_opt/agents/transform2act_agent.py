...
class Transform2ActAgent(AgentPPO):
    ...
    def setup_logger(self):
        ...
        run_name = f"{self.cfg.env_name}_seed{self.cfg.seed}_{time.strftime('%Y%m%d-%H%M%S')}"
        self.tb_logger = SummaryWriter(os.path.join(cfg.tb_dir, run_name)) if self.training else None
    ...
    def optimize(self, epoch):
        ...
        return info['log_eval'].avg_exec_episode_reward
    ...
    def run_and_save_design(self, num_episode=1, epoch=0, mean_action=True, save=False, xml_path=None):
        env = self.env
        for i in range(num_episode):
            ...
            for t in range(10000): # max episode length
                ...
                if done:
                    if save:
                        xml_path = f'{self.cfg.model_dir}/{epoch}.xml'
                        with open(xml_path, 'w') as f:
                            f.write(env.cur_xml_str)
                        self.logger.info(f"Saved xml to {xml_path}")
                        json_path = f'{self.cfg.model_dir}/{epoch}.json'
                        info_dict = {
                            'xml': env.cur_xml_str,
