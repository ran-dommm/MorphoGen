...
class AntEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, cfg, agent):
        ...
        self.agent = agent
        self.model_xml_file = cfg.xml_path if hasattr(cfg, 'xml_path') and cfg.xml_path else 'assets/mujoco_envs/ant.xml'
        print(f"Using model XML file: {self.model_xml_file}")
        # robot xml
        ...
        self.index_base = 5
        self.stage = 'skeleton_transform' if not cfg.ctrl_only else 'execution'    # transform or execute
    ...
    def step(self, a):
        ...
        if self.stage == 'skeleton_transform':
            # print("skeleton transform stage")
            skel_a = a[:, -1]
        ...
        # attribute transform stage
            ...
            # print("attribute transform stage")
        ...
        # execution stage
            ...
            xposbefore = self.get_body_com(self.robot.bodies[0].name)[0]
            ...
            xposafter = self.get_body_com(self.robot.bodies[0].name)[0]
            ...
            ob = self._get_obs()
            return ob, reward, done, {'use_transform_action': False, 'stage': 'execution', 'ctrl': ctrl}
    ...
    def get_sim_obs(self):
        ...
        for i, body in enumerate(self.robot.bodies):
            ...
            if i == 0:
                ...
                qs, qe = get_single_body_qposaddr(self.model, body.name_str)
                if qe - qs >= 1:
                    assert qe - qs == 1
    ...
    def get_attr_design(self):
        ...
        try:
            ...
            for obs_i in obs:
                print(obs_i.shape)
                raise Exception("Error in get_attr_design")
        return obs
    ...
    def reset_model(self):
        self.reset_robot()
        self.control_nsteps = 0
        self.stage = 'skeleton_transform' if not self.cfg.ctrl_only else 'execution'
        self.cur_t = 0
