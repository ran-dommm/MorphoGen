...
class Joint:
    ...
    def __init__(self, node, body):
        ...
        # print(f"Joint {self.name} position: {self.pos}")
        # print(f"Body {body.name} position: {body.pos}")
...
class Body:
    ...
    def __init__(self, node, parent_body, robot, cfg):
        ...
        if 'name' in node.attrib:
            ...
            self.name = self.parent.name + f'_child{len(self.parent.child)}'
        else:
            self.name = str(Body.cnt)
            Body.cnt += 1
...
class Robot:
    ...
    def load_from_xml(self, xml, is_xml_str=False):
        ...
        # print(BytesIO(xml) if is_xml_str else xml)
