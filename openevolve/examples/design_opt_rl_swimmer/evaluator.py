    ...
    try:
        ...
        if match:
            try:
                ...
                print(f"Training completed. Best reward from BEST_EVAL_RESULT: {best_reward}")
                return best_reward
            except ValueError:
                print("Could not parse BEST_EVAL_RESULT.")
...
def evaluate_design(xml_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Evaluation completed. Best reward: {best_reward}")
        return best_reward if best_reward != -1e9 else 0.0
