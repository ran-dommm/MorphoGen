...
def main():
    ...
    parser.add_argument("--config", "-c", help="Path to config file", 
                       default="./openevolve/examples/design_opt_rl_swimmer/config.yaml")
    parser.add_argument("--output", "-o", help="Output directory", 
                       default="./top_programs")
    ...
    if not os.path.exists(args.checkpoint_path):
        print(f"Error: Checkpoint directory '{args.checkpoint_path}' not found")
        return 1
    ...
    try:
        ...
        database = load_checkpoint_database(args.checkpoint_path, args.config)
        ...
        # Save programs
        save_programs(top_programs, args.output, args.file_extension)
        
        print(f"Successfully extracted {len(top_programs)} programs to {args.output}")
