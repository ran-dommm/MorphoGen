...
TARGET_SCORE=""
LOG_LEVEL="INFO"
OUTPUT_DIR="./openevolve/examples/design_opt_rl_swimmer/result/swimmer_new"
EXTRACT_TOP=""
...
export OPENEVOLVE_ROBOT_XML="./openevolve/examples/design_opt_rl_swimmer/init_xml/best.xml"
...
if [ -n "$EXTRACT_TOP" ]; then
    ...
    if [ -d "$OUTPUT_DIR/checkpoints" ]; then
        ...
        if [ -n "$LATEST_CHECKPOINT" ]; then
            ...
            python ./openevolve/examples/design_opt_rl_swimmer/extract_top_programs.py \
                "$CHECKPOINT_PATH" \
                --config "./openevolve/examples/design_opt_rl_swimmer/config.yaml" \
