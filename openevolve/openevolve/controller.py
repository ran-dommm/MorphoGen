...
class OpenEvolve:
    ...
    def __init__(
        ...
        self.output_dir = os.path.join(
            os.path.dirname(initial_program_path), output_dir
        ) or os.path.join(
    ...
    async def _load_elite_seeds(self, start_iteration: int) -> None:
        """
        Load elite seed programs to populate the initial population
        
        Args:
            start_iteration: Current iteration number
        """
        elite_config = self.config.elite_seeds
        seed_files = elite_config.files
        
        logger.info(f"Loading {len(seed_files)} elite seed programs")
        
        loaded_count = 0
        for seed_file in seed_files:
            try:
                # Load seed program code
                ...
                # Generate unique ID for seed program
                seed_program_id = str(uuid.uuid4())
                
                # Evaluate the seed program
                logger.info(f"Evaluating elite seed: {os.path.basename(seed_file)}")
                seed_metrics = await self.evaluator.evaluate_program(seed_code, seed_program_id)
                
                # Create seed program object
                seed_program = Program(
                    ...
                    metadata={"seed": True, "seed_file": os.path.basename(seed_file)}
                )
                
                # Add to database
                self.database.add(seed_program, target_island=(loaded_count+1)%self.config.database.num_islands)
                loaded_count += 1
                
                # Log seed program metrics
                logger.info(f"Added elite seed '{os.path.basename(seed_file)}' with metrics: {seed_metrics}")
                
            except Exception as e:
                logger.error(f"Failed to load elite seed {seed_file}: {str(e)}")
                continue
        
        logger.info(f"Successfully loaded {loaded_count}/{len(seed_files)} elite seed programs")
    ...
    async def run(
        ...
        # Load elite seed programs if configured and starting fresh
        if start_iteration == 0 and hasattr(self.config, 'elite_seeds') and self.config.elite_seeds.enabled:
            await self._load_elite_seeds(start_iteration)
