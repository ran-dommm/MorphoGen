...
class PromptSampler:
    ...
    def _get_gradient(self, current_program: str, program_metrics: Dict[str, float], critic_config: Dict[str, Any]) -> str:
        """
        Use an LLM as a critic to provide an improvement gradient.
        """
        if not critic_config.get("enabled"):
            return None
        else:
            api_key = critic_config.get("api_key", "")
            api_base = critic_config.get("api_base", "")
            if not api_key or not api_base:
                return None
            else:
                critic_client = openai.OpenAI(api_key=api_key, base_url=api_base)

        try:
            aspects = critic_config.get("aspects", [])
            if not aspects:
                return None

            if 'locomotion_reward' not in program_metrics:
                prompt = f"""
                        You are a robotics expert acting as a critic for an evolutionary algorithm designing a MuJoCo robot.
                        Your task is to provide a specific, actionable suggestion for how to improve the robot's design.

                        The robot's current XML structure is:
                        ```xml
                        {current_program}
                        ```

                        Provide a concise suggestion for improvement focusing specifically on one aspect from {aspects}.
                        Do not give general or too specific advice. Your suggestion should be a concrete change to the XML structure related to the chosen aspect.

                        Example for "limb lengths and orientations": "Try increasing the length of the front leg segments."
                        Example for "body segments": "Consider adding a new body segment or deleting an existing body segment to the torso for more flexibility."
                        Example for "body positions and orientations": "Adjust the initial orientation of the main torso to be more parallel to the ground."
                ...
                prompt = f"""
                            ...
                            - Locomotion Reward: {program_metrics.get('locomotion_reward', 'N/A')}
                            - Normalized Locomotion: {program_metrics.get('normalized_locomotion', 'N/A')}
                            - Complexity Score: {program_metrics.get('complexity_score', 'N/A')}
                            - Actuator Efficiency: {program_metrics.get('actuator_efficiency', 'N/A')}
                            - Number of Bodies: {program_metrics.get('num_bodies', 'N/A')}
                            - Number of Joints: {program_metrics.get('num_joints', 'N/A')}

                            Based on this information, provide a concise suggestion for improvement focusing specifically on one aspect from {aspects}.
                            Do not give general or too specific advice. Your suggestion should be a concrete change to the XML structure related to the chosen aspect.
            ...
            response = critic_client.chat.completions.create(
                model=critic_config.get("model", "qwen-plus"),
                messages=[
                    {"role": "system", "content": "You are a robotics expert in structural design acting as a critic."},
            ...
            # Sanitize the output
            gradient = gradient.replace('"', '').replace("'", '').strip()
            
            print(f"LLM Critic Suggestion: {gradient}")
            return gradient

        except Exception as e:
            print(f"Could not get gradient from LLM: {e}")
            traceback.print_exc()
            return None
    ...
    def build_prompt(
        ...
        **kwargs: Any,
        ...
        # Extract gradient from kwargs for the critic
        gradient = self._get_gradient(current_program, program_metrics, self.config.critic)
        # gradient = None
    ...
    def _identify_improvement_areas(
        ...
        # Add LLM critic's gradient if provided
        if gradient:
            # This uses a new template fragment 'llm_critic_gradient'
            # You will need to add this to your template fragments.
            # For example: "LLM critic suggests focusing on: {gradient}"
            try:
                ...
                # Fallback if the fragment is not defined
                logger.warning("Template fragment 'llm_critic_gradient' not found. Using fallback.")
                improvement_areas.append(f"LLM critic suggests focusing on: {gradient}")
    ...
    def _format_evolution_history(
        ...
        selected_previous = previous_programs[-min(1, len(previous_programs)) :]
